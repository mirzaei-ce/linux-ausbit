...
###   This script attempts to download the signature file SHA256SUMS.asc from ausbit.org
...
WORKINGDIR="/tmp/ausbit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://ausbit.org/bin/"
VERSIONPREFIX="ausbit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'ausbit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Ausbit Core binary release signing key installed?"
   fi
