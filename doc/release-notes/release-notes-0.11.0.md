Ausbit Core version 0.11.0 is now available from:

  <https://ausbit.org/bin/ausbit-core-0.11.0/>
...
backwards-compatible with pre-0.10 versions of Ausbit Core or other software:
...
For example, add the following to `ausbit.conf`:
...
related to the blockchain in the ausbit system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Ausbit Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Ausbit Core will 
...
For an example script see [ausbit-submittx](https://github.com/laanwj/ausbit-submittx).
...
- #5334 `cf87536` libausbitconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new ausbit-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only AusbitAmountField
- #5753 `0f44672` Add ausbit logo to about screen
...
- #5809 `46bfbe7` Add ausbit-cli man page
...
- #5849 `41259ca` contrib/init/ausbitd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Ausbit-Qt.app to Ausbit-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for ausbit-tx)
